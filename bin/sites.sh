...
#   "; HugoCMS – Mounts for <HOST> …", or "Mounts für …" with --lang=de); the
#   hash in the filename is not reversible.
#
...
# Read the host from the mount file's header comment (install.sh format). The
# header may be English or German (install.sh --lang); the pattern accepts both
# ("Mounts for … (generated by …" and "Mounts für … (von install.sh …").
read_mount_host() {
